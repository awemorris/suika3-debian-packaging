...
find "$SRC_DIR" \
    -name ".vscode" -prune -o \
    -name "misc" -prune -o \
    -name "suika3.exe" -false -o \
    -name "assets.arc" -false -o \
...
if [ "$OS" == "Linux" ]; then
    if [ ! -d commandlinetools-linux-11076708_latest ]; then
...
if [ "$OS" == "Darwin" ]; then
    if [ ! -d commandlinetools-mac-11076708_latest ]; then
...
export SDK_TEMP_ROOT=$CURRENT_DIR/android-sdk
yes | ./cmdline-tools/bin/sdkmanager --install "cmdline-tools;latest" --sdk_root=$SDK_TMP_ROOT
set ANDROID_SDK_ROOT=$SDK_TEMP_ROOT
set ANDROID_HOME=$SDK_TEMP_ROOT
set NEW_SDKMANAGER=$ANDROID_SDK_ROOT/cmdline-tools/latest/bin/sdkmanager
yes | $NEW_SDKMANAGER --install --licenses --sdk_root=$ANDROID_SDK_ROOT
...
echo
export PACKAGE_NAME=`$ANDROID_HOME/build-tools/35.0.0/aapt2.exe dump packagename $CUR_DIR/app/build/outputs/apk/debug/app-debug.apk`
...
if [ "$s" == "y" ]; then
    ...
    $ANDROID_HOME/platform-tools/adb.exe uninstall $PACKAGE_NAME

    echo Installing new APK...
    $ANDROID_SDK_ROOT/platform-tools/adb.exe install -r $CUR_DIR/app/build/outputs/apk/debug/app-debug.apk

    echo Starting app...
    $ANDROID_HOME/platform-tools/adb.exe shell am start -n $PACKAGE_NAME/io.noctvm.playfield.engineandroid.MainActivity
fi
